use crate::utils;

fn part1(data: &str) -> u32 {
    let mut total_safe_reports: u32 = 0;
    let mut number_arr: Vec<Vec<u32>> = vec![];
    let reports = data.lines();
    for report in reports {
        number_arr.push(
            report
                .split(" ")
                .map(|number_str| number_str.parse::<u32>().expect("Failed to parse number"))
                .collect::<Vec<u32>>(),
        );
    }

    let mut interruptions = 0;
    for report in number_arr {
        interruptions = 0;
        if report.first().expect("Couldn't get first number")
            > report.last().expect("Couldn't get last number")
        {
            // THIS IS DECREASING
            for i in 0..report.len() - 1 {
                if report[i].abs_diff(report[i + 1]) > 3
                    || report[i].abs_diff(report[i + 1]) == 0
                    || report[i] < report[i + 1]
                {
                    interruptions += 1;
                    break;
                }
            }
        }
        if report.first().expect("Couldn't get first number")
            < report.last().expect("Couldn't get last element")
        {
            for i in 0..report.len() - 1 {
                if report[i].abs_diff(report[i + 1]) > 3
                    || report[i].abs_diff(report[i + 1]) == 0
                    || report[i] > report[i + 1]
                {
                    interruptions += 1;
                    break;
                }
            }
        }
        if report.first().expect("Couldn't get first number")
            == report.last().expect("Couldn't get last element")
        {
            interruptions += 1;
        }
        if interruptions == 0 {
            total_safe_reports += 1;
        }
    }

    total_safe_reports
}

fn part2(data: &str) -> u64 {
    let mut left_row: Vec<u64> = vec![];
    let mut right_row: Vec<u64> = vec![];
    let _: Vec<_> = data
        .lines()
        .map(|line| {
            let temp_arr: Vec<&str> = line.trim().split("   ").collect();
            left_row.push(
                temp_arr[0]
                    .parse::<u64>()
                    .expect("Cannot parse the left row number"),
            );
            right_row.push(
                temp_arr[1]
                    .parse::<u64>()
                    .expect("Cannot parse the right row number"),
            );
        })
        .collect();

    let mut total_similarity: u64 = 0;
    let mut total_times;
    for &number_l in &left_row {
        total_times = 0;
        for &number_r in &right_row {
            if number_r == number_l {
                total_times += 1;
            }
        }
        total_similarity += number_l * total_times;
    }
    total_similarity
}

pub fn day02(day: u8, suffix: Option<&str>, part: u8) {
    let data = utils::load_file(day, suffix);
    match part {
        1 => println!("Output of part #1: {}", part1(&data)),
        2 => println!("Output of part #2: {}", part2(&data)),
        3 => println!("Output of both parts will show!"),
        _ => println!("Wrong input, cannot show output of any part!"),
    }
}
